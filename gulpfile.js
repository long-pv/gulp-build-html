...
function styles() {
	return src(paths.styles.src).pipe(sass(SASS_OPTIONS).on("error", sass.logError)).pipe(cleanCSS()).pipe(rename("main.css")).pipe(dest(paths.styles.dest)).pipe(browserSync.stream());
}
